...
import { Users, UserCheck, UserX, LayoutDashboard, PieChart as PieIcon } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ servers }) => {
  ...
  const renderChartSection = (title: string, data: any[], icon: any, colorClass: string) => (
    <div className="card-chrome p-5 rounded-[2.5rem] flex flex-col h-full">
            ...
            <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                    <Pie
                        ...
                        dataKey="value"
                        labelLine={false}
                        label={false} // Se eliminan las etiquetas sobre el gráfico
                    >
      ...
      <div className="grid grid-cols-1 gap-2 mt-4">
        {data.map(d => (
          <div key={d.name} className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800/50">
            <div className="flex items-center gap-2 min-w-0">
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{backgroundColor: d.color}}></span>
                <p className="text-[9px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider truncate">{d.name}</p>
            </div>
            <p className="text-[11px] font-black text-slate-900 dark:text-slate-100 ml-2">{d.value}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-6 pb-24 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {renderChartSection("Activos por Grupo", activeGroupData, UserCheck, "bg-blue-600")}
        {renderChartSection("Inactivos por Grupo", inactiveGroupData, UserX, "bg-rose-600")}
      </div>
    </div>
  );
};
